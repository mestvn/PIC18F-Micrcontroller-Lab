...
void Do_Setup_Alarm_Time(void)
{
    ...
    {   
         if (check_for_button_input() == 1)
         {
             ...
             {
                Do_Beep_Good();
                if (found == Prev) Go_Prev_Alarm_Field();           // if 'Prev' then use Go_Prev_Alarm_Field();  
                if (found == Next) Go_Next_Alarm_Field();           // if 'Next' then use Go_Next_Alarm_Field();  
                if (found == Play_Pause) Do_Save_New_Alarm_Time();  // if 'Play/Pause' then use Do_Save_New_Alarm_Time();
                if (found == Minus) Decrease_Alarm_Time();          // if 'Minus' then use Decrease_Alarm_Time();
                if (found == Plus) Increase_Alarm_Time();           // if 'Plus' then use Increase_Alarm_Time();
                if (found == EQ) Exit_Setup_Alarm_Time();           // if 'EQ' Exit_Setup_Alarm_Time();
                found = 0xff;
             }
             else
             {
...
{ 
    ...
    strcpy(txt, "ECE3301L Fa'22 S1");                               // Text displayed 
