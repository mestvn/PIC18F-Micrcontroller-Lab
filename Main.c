...
char buffer[31]     = " ECE3301L Fa'22 S1 \0";
char *nbr;
char *txt;
char tempC[]        = "+25";
char tempF[]        = "+77";
...
{ 
    ...
    ADCON1=0x0D;                                                // AN1 
    TRISA = 0x02;                                               // only AN1
    TRISB = 0x07;                                               // 3 interrupts
    TRISC = 0x01;                                               // Tach Pulse
    TRISD = 0x00;
    TRISE = 0x00;
    PORTE = 0x00;
...
void main() 
{
    ...
    tempSecond = 0xff;
    Set_Alarm_RGB(7);                                           // Initialize Alarm RGB
    ...
    while (1)
    {
        ...
        {
            INT1_flag = 0;
            if (ALARMEN == 0) ALARMEN = 1;                  // toggles alarm
            else ALARMEN = 0;
        }
...
void Test_Alarm()
{
    ...
    else
    {
        if (ALARMEN == 0)
        {
            DS3231_Turn_Off_Alarm();
            MATCHED = 0;
            alarm_mode = 0;                                 // turn off alarm but leave sw on
            Set_Alarm_RGB(7);                               // turn off RGB LED
            Deactivate_Buzzer();                            // turn off Buzzzer
        }
        else
        {            
            if(RTC_ALARM_NOT == 0)
            {
                ...
                if(MATCHED == 1)
                {    
                    Set_Alarm_RGB(i);                       // incrementing color
                    if (i < 7) i++;                         // loop without using for loop since while loops for us
                    else i= 0;
                    Activate_Buzzer_4KHz();
                    if (volt > 1.0)                         // moved lover voltage for waving over
                    {
                        MATCHED = 0;                        
                        Set_Alarm_RGB(7);                   // turn off RGB LED
