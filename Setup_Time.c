...
void Do_Setup_Time(void)
{
    ...
    {   
         if (check_for_button_input() == 1)
         {
             ...
             {
                Do_Beep_Good(); 
                if (found == Prev) Go_Prev_Field();                 // if 'Prev' then use Go_Prev_Field();  
                if (found == Next) Go_Next_Field();                 // if 'Next' then use Go_Next_Field();  
                if (found == Play_Pause) Do_Save_New_Time();        // if 'Play/Pause' then use Do_Save_New_Time();
                if (found == Minus) Decrease_Time();                // if 'Minus' then use Decrease_Time();
                if (found == Plus) Increase_Time();                 // if 'Plus' then use Increase_Time();
                if (found == EQ) Exit_Time_Setup();                 // if 'EQ' Exit_Time_Setup();
                found = 0xff;
             }
             else
             {
...
void Increase_Time()
{
            switch (Select_Field)                                   // Increment the proper field
            {
                ...
                case 5:
                    setup_year++;
                    if (setup_year == 100) setup_year = 1;
                    break;
                default:
                    break;
            }    
