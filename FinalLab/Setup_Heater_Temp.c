...
In_Heater_Temp_Setup = 1;
    ...
    while (In_Heater_Temp_Setup == 1)
    {
         if (check_for_button_input() == 1)
        {
             ...
             {
                Do_Beep_Good(); 
                if (found == Play_Pause) Do_Save_New_Heater_Temp(); // if 'Play/Pause' then use Do_Save_New_Heater_Temp();
                if (found == Minus) Decrease_Heater_Temp();         // if 'Minus' then use Decrease_Heater_Temp();
                if (found == Plus) Increase_Heater_Temp();          // if 'Plus' then use Increase_Heater_Temp();
                if (found == EQ) Exit_Setup_Heater_Temp();          // if 'EQ' Exit_Setup_Heater_Temp();
                found = 0xff;
             }
             else
             {
...
{
    setup_heater_set_temp++;                                        // increase setup_heater_set_temp by 1
    if (setup_heater_set_temp > 120) setup_heater_set_temp = 120;   // if it is greater than 120. If so, limit to 120
...
{
    --setup_heater_set_temp;                                        // decrease setup_heater_set_temp by 1
    if (setup_heater_set_temp < 50) setup_heater_set_temp = 50;     // if it is less than 50. If so, limit to 50
    Update_Setup_Heater_Screen();                                   // Update screen with latest info       
}
...
{ 
    ...
    strcpy(txt, "ECE3301L Fa'22 S1\0");                             // Text displayed 
...
{
    char dc_char1 = setup_heater_set_temp/100;                      // normal calculation
